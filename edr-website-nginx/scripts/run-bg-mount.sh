...
echo $DOCKER run --name $CONTAINER_NAME -dp80:80 -v "$(pwd)"/html:/usr/share/nginx/html/ $IMAGE_NAME
$DOCKER run --name $CONTAINER_NAME -dp80:80 -v "$(pwd)"/src:/usr/share/nginx/html/ $IMAGE_NAME
